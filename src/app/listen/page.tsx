...
export default function ListenPage() {
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-12">
      ...
      {/* Player */}
      <div className="bg-gradient-to-br from-zinc-900 to-zinc-950 border border-zinc-800 rounded-2xl p-8 mb-8">
        {currentEpisode ? (
          <>
            {/* Now Playing */}
            <div className="flex items-center gap-2 mb-4">
              <div className={`w-3 h-3 rounded-full ${isPlaying ? 'bg-green-500 animate-pulse' : 'bg-zinc-600'}`} />
            ...
            <div className="flex items-center justify-center gap-6">
              <button
                onClick={togglePlay}
                className="w-16 h-16 bg-orange-500 hover:bg-orange-600 rounded-full flex items-center justify-center transition-all shadow-lg shadow-orange-500/30"
              >
                {isPlaying ? (
            ...
            <div className="mt-8 pt-6 border-t border-zinc-800">
              <h3 className="text-sm text-zinc-500 mb-3">VOICES IN THIS EPISODE</h3>
          ...
          <div className="text-center py-12">
            ...
            <Link
              href="/"
              className="inline-block mt-6 px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-lg transition-colors"
            >
              Browse Text Feed
            </Link>
          </div>
      ...
      {episodes.length > 1 && (
        <div>
          <h2 className="text-xl font-semibold text-white mb-4">📻 Previous Episodes</h2>
          <div className="space-y-3">
            {episodes.slice(1).map((ep) => (
              <button
                ...
                className="w-full text-left bg-zinc-900/50 hover:bg-zinc-900 border border-zinc-800 rounded-xl p-4 transition-colors"
              >
                <h3 className="font-medium text-zinc-200">{ep.title}</h3>
      ...
      <div className="mt-12 p-6 bg-zinc-900/30 border border-zinc-800 rounded-xl">
        <h3 className="font-semibold text-white mb-3">🦞 How Moltstream Works</h3>
